...
const AddComment = ({ post }) => {
  ...
  const handleComment = (event) => {
    ...
    fetch("https://server-atg-mern.vercel.app/addComment", {
      method: "POST",
      headers: {
  ...
  useEffect(() => {
    fetch(`https://server-atg-mern.vercel.app/addComment/${post?._id}`)
      .then((res) => res.json())
      .then((result) => {
      ...
      <form onSubmit={handleComment}>
        <div className="flex items-center px-3 py-2 rounded-lg bg-gray-50 dark:bg-gray-200">
          <textarea
            rows="1"
            name="commentContent"
            className="block mx-4 p-2.5 w-full text-sm text-gray-900 bg-white rounded-lg border border-gray-300 dark:bg-gray-200 dark:border-gray-600 dark:placeholder-gray-400 dark:text-gray-900 "
            placeholder="Your comment..."
          ></textarea>

          <button
            type="submit"
            className="inline-flex justify-center p-2 text-green-600 rounded-full cursor-pointer  hover:text-green-700"
          >
